import React, { Component } from 'react'
import { connect } from 'react-redux'
import {Redirect} from 'react-router'
import {Link} from 'react-router-dom'

import { userInput, userPassword, registerUser } from '../../reducers/reducer'

class Auth extends Component {
        

    render() {
        const { userInput, userPassword, redirect} = this.props;
        if (redirect){
            return<Redirect to ="/Dashboard" />;
        }
        console.log(redirect)
        return (
            <div className="main-auth-container">
            <div className="auth-box">
            <img className="auth-logo" src="data:image/png;base64," alt="logo"/>
            <h1> Helo </h1>
            <div className="username-input">
                    <input placeholder="username" value={this.props.username} onChange={(e) => userInput(e.target.value)}></input>
                </div>
                <div className="password-input">
                    <input placeholder="password" value={this.props.password} onChange={(e) => userPassword(e.target.value)}></input>
                </div>
                <div className="button-container">
                    <button> Login </button>
                    <Link to ="/Dashboard" > <button onClick={()=>this.props.registerUser(this.props.username, this.props.password)}> Register </button> </Link>
                </div>
                </div>
            </div>
        )
    }
}

function mapStateToProps(state) {
    return {
        username: state.username,
        password: state.password,
        redirect: state.redirect
    }
}

export default connect(mapStateToProps, { userInput, userPassword, registerUser })(Auth)